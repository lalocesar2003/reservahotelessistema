...
<style>
	...
	.menu-carrusel {
		overflow-x: auto;
		overflow-y: hidden;
		-webkit-overflow-scrolling: touch; /* Scroll táctil en móviles */
		scrollbar-width: none; /* Oculta scrollbar Firefox */
		-ms-overflow-style: none; /* Oculta scrollbar IE/Edge */
		scroll-behavior: smooth; /* Animación de deslizamiento suave */
		flex-grow: 1;
	}
		...
		<div class="row">
			...
			<div class="col-12 col-lg-8 colIzqHabitaciones p-0">
				...
				<div class="pt-4 cabeceraHabitacion">
					...
					<?php 
						// Contamos cuántos estilos/habitaciones hay
						$cantidadHabitaciones = count($habitaciones); 
					?>

					<div class="<?php echo ($cantidadHabitaciones > 4) ? 'menu-carrusel-wrapper mt-lg-4 px-3' : 'mt-lg-4'; ?>">
						
						<?php if($cantidadHabitaciones > 4): ?>
							<button type="button" class="btn-scroll-menu btn-scroll-left" onclick="scrollMenuEstilos(-200)">
								<i class="fas fa-chevron-left"></i>
							</button>
						<?php endif; ?>

						<ul id="menuEstilos" class="nav <?php echo ($cantidadHabitaciones > 4) ? 'flex-nowrap menu-carrusel' : 'nav-justified w-100'; ?>">	

							<?php foreach ($habitaciones as $key => $value): ?>

							<li class="nav-item" <?php if($cantidadHabitaciones > 4) echo 'style="min-width: 140px; text-align: center;"'; ?>>
						...
						<?php if($cantidadHabitaciones > 4): ?>
							<button type="button" class="btn-scroll-menu btn-scroll-right" onclick="scrollMenuEstilos(200)">
				...
				<section class="jd-slider mb-3 my-lg-3 slideHabitaciones">
			        ...
			        <div class="slide-inner">
				  	...
				  	<a class="prev d-none d-lg-block" href="javascript:void(0);">
					    <i class="fas fa-angle-left fa-2x"></i>
					</a>

					<a class="next d-none d-lg-block" href="javascript:void(0);">
...
<?php if($cantidadHabitaciones > 4): ?>
<script>
	function scrollMenuEstilos(cantidad) {
		const menu = document.getElementById('menuEstilos');
		if (menu) {
			// Desplaza el contenedor horizontalmente
			menu.scrollBy({ left: cantidad, behavior: 'smooth' });
		}
	}
</script>
<?php endif; ?>