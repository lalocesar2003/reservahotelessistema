			...
			if(preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["registroNombre"]) &&
			   preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["registroEmail"]) &&
				...
				if($respuesta == "ok"){
					...
					$mail->isSMTP();
				$mail->Host = app_env('SMTP_HOST');
$mail->SMTPAuth = true;
$mail->Username = app_env('SMTP_USERNAME');
$mail->Password = app_env('SMTP_PASSWORD');
$mail->SMTPSecure = app_env('SMTP_ENCRYPTION');
$mail->Port = (int) app_env('SMTP_PORT');
//  $mail->SMTPDebug = 2;  // Modo de depuración detallado
// $mail->Debugoutput = 'html';
				$mail->setFrom(app_env('SMTP_FROM_ADDRESS'), app_env('SMTP_FROM_NAME'));


				$mail->addReplyTo(app_env('SMTP_FROM_ADDRESS'), app_env('SMTP_FROM_NAME'));
...
$mail->Debugoutput = 'html';
				...
				$mail->addAddress(app_env('SMTP_FROM_ADDRESS'));
