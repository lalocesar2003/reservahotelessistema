...
class ModeloHabitaciones{
	...
	// ... tu código anterior (mdlEliminarHabitacion) ...

    /*=============================================
    MOSTRAR MAPA DE DISPONIBILIDAD DE HOY
    =============================================*/
    static public function mdlMostrarDisponibilidadHoy(){
        $stmt = Conexion::conectar()->prepare("
            SELECT 
                ...
                END AS estado
            FROM categorias c
            INNER JOIN habitaciones h ON c.id = h.tipo_h
            LEFT JOIN reservas r ON h.id_h = r.id_habitacion 
                AND CURDATE() >= r.fecha_ingreso 
                AND CURDATE() < r.fecha_salida
            ORDER BY c.id DESC, h.id_h ASC
        ");
        $stmt -> execute();
        return $stmt -> fetchAll();
        $stmt -> close();
